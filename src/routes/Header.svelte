...
<header class="bg-white shadow-lg h-24 hidden md:flex">
	...
	<nav class="header-links contents font-semibold text-base lg:text-lg">
		<ul class="flex items-center ml-4 xl:ml-8 mr-auto list-none">
			...
			<li class="p-3 xl:p-6 relative">
				<a href="/game" on:focus={gameOpen} on:mouseover={gameOpen}>
					<span>{$_('games')}</span>
					<i class="fa-solid fa-chevron-down text-xs text-gray-400" />
				</a>
				{#if showGame}
		...
		<ul class="flex items-center mr-4 lg:mr-6 xl:mr-8 list-none">
			...
			<li class="p-1">
				<a
					...
					class="inline-block rounded-full border p-2 px-2.5 hover:shadow-lg hover:border-opacity-0 duration-200 hover:-translate-y-0.5">
					<svg
						class="h-4"
						aria-hidden="true"
						focusable="false"
	...
	<div class="border flex items-center px-4 lg:px-6 xl:px-8">
		<a href="#contact" class="mr-4 lg:mr-6 xl:mr-8">
			<svg
				class="h-6 xl:h-8"
				aria-hidden="true"
				focusable="false"
...
<header class="md:hidden bg-white w-full mobile-nav">
	...
	<nav
		class="mt-5 relativefont-semibold text-base lg:text-lg max-h-0 transition-all duration-300 overflow-auto border-t {showMobileMenu &&
			'max-h-96'}">
		<ul class="list-none">
			<li class="p-3 xl:p-6 w-full border {currentPath === '/' && 'active'}">
				<a href="/" class="p-3 w-full">
					<span>{$_('home')}</span>
				</a>
			</li>
			...
			<li class="p-3 xl:p-6 border {currentPath === '/ingredient' && 'active'}">
				<a href="/ingredient" class="p-3 w-full">
					<span>{$_('ingredients')}</span>
				</a>
			</li>
			<li class="xl:p-6 border w-full relative">
